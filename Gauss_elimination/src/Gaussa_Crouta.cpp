    ...
    for (int k = 0; k < n; k++) {
        ...
        for (int i = k + 1; i < n; i++) {
            if (abs(A[i][k]) > abs(A[max_row][k])) {
                max_row = i;                                                         /// wybieramy index elementu w kolumnie o największej wartości bezwzględnej
            }
        }
        /// zamiana wierszy
        swap(A[k], A[max_row]);
...
void rozwiaz_uklad_2(vector<vector<double>>& wsp, vector<double>& wyr_wol) {
    ...
    vector<double> x = gauss_elimination_2(wsp,wyr_wol);                                 ///  Sprowadzenie do macierzy schodkowej
    ...
    if(rzad_normalnej == n && n  == rzad_uzupelnionej && rzad_normalnej != -1)  {
        cout << "Gauss_Crouta: Rozwiązanie układu równań:\n";
        for (int i = 0; i < x.size(); i++) {
            cout << "x[" << i << "] = " << x[i] << "\n";
        }
    }
    else if (rzad_normalnej != rzad_uzupelnionej && rzad_normalnej != -1 ) {
        cout << "Gauss_Crouta: Układ sprzeczny." << endl;
    }
    else if (rzad_normalnej == rzad_uzupelnionej && rzad_uzupelnionej < n && rzad_normalnej != -1) {
        cout << "Gauss_Crouta: Układ nieoznaczony, zalezny od: " << n -  rzad_normalnej << " parametrów." << endl;
    }
    else {
        cout << "Gauss_Crouta: NaN error" << endl;
    }
}
